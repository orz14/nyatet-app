...
<div class="w-full max-w-md overflow-hidden">
    ...
    <form method="POST" action="{{ route('login') }}" autocomplete="off">
        ...
        <div class="flex items-center justify-between w-full max-w-sm mx-auto mt-4 mb-5 text-sm">
            ...
            <a href="{{ route('redirect', 'password.request') }}" class="text-teal-700 underline transition hover:text-teal-500 decoration-2 decoration-teal-500/30">{{ __('Lupa Password?') }}</a>
            @endif
        </div>
    ...
    <div class="mb-1 text-sm text-center text-slate-500">
        {{ __('Belum memiliki akun?') }} <a href="{{ route('redirect', 'register') }}" class="text-teal-700 underline transition hover:text-teal-500 decoration-2 decoration-teal-500/30">{{ __('Daftar') }}</a>
    </div>
</div>
@endsection
