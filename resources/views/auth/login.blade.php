...
@section('content')
    <div class="w-full max-w-md overflow-hidden">
        ...
        <form method="POST" action="{{ route('login') }}" autocomplete="off">
            @csrf
            {{-- Username --}}
            <x-form-auth :name="__('username')" :ph="__('Masukkan Username')" :value="old('username')" required
                autofocus>{{ __('Username') }}</x-form-auth>
            ...
            <div class="flex items-center justify-between w-full max-w-sm mx-auto mt-4 mb-5 text-sm">
                ...
                <div class="inline-flex items-center gap-x-2">
                    <input
                        class="text-teal-700 transition-all duration-300 ease-in-out bg-white border-teal-700 rounded shadow-sm focus:border-teal-700 focus:ring focus:ring-teal-600/20 focus:ring-opacity-80"
            ...
            <x-auth-button :label="__('Masuk')">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-4 h-4 mr-1 md:w-5 md:h-5">
        ...
        @if (env('ENABLE_SOCIAL_LOGIN'))
            <div class="w-full max-w-sm mx-auto divider before:bg-slate-300/70 after:bg-slate-300/70 text-slate-500">atau
            </div>
        ...
        <div class="mb-1 text-sm text-center text-slate-500">
            {{ __('Belum memiliki akun?') }} <a href="{{ route('register') }}"
                class="text-teal-700 underline transition hover:text-teal-500 decoration-2 decoration-teal-500/30"
                wire:navigate.hover>{{ __('Daftar') }}</a>
        </div>
    </div>
@endsection
